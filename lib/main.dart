...
class _WeatherAppState extends State<WeatherApp> {
  ...
  List<int> temperatureAtHour = List.filled(6, 0);
  List<String> abbreviationHourForecast = List.filled(6, '');

  String location = 'Kolkata';
  int woeid = 2295386;
  String weather = 'clear';
  String abbreviation = '';
  List<String> abbreviationForecast = List.filled(5, '');
  String errorMessage = '';
  ...
  void fetchHourlyForecast() async {
    ...
    for (var i = 0; i < 5; i++) {
      var locationDayResult = await http.get(locationApiUrl +
          woeid.toString() +
          '/' +
          new DateFormat('y/M/d').format(today).toString());
      var result = json.decode(locationDayResult.body);
      var data = result[0];

      setState(() {
        temperatureAtHour[i] = data["the_temp"].round();
        abbreviationHourForecast[i] = data["weather_state_abbr"];
      });
    }
  }
                      ...
                      hourFlag
                          ? SingleChildScrollView(
                              ...
                              child: Row(
                                children: <Widget>[
                                  for (var i = 0; i < 5; i++)
                                    hourForecastElement(i, temperatureAtHour[i],
...
Widget hourForecastElement(time, temperatureAtHour, abbreviation) {
  var hourList = ['9pm', '6pm', '3pm', '12pm', '9am'];
  return Padding(
